...
2. **VS Code** — the second track

The landing page is a persuasive/read hybrid with a full-viewport hero, a signal board showing two route stops (App and VS Code), route-line SVG notation, and geometric block lab cards below. The Starlight doc shell provides calm, legible lesson pages.

### Signal board

The hero's signal board uses a grid background, a Copilot App session fragment, and two surface cards (App and VS Code) connected by a route-line SVG — conveying a wayfinding metaphor rather than generic cards.
